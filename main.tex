...
%KD: Do I understand this correctly: The S72X mutation screen was done on a precursor to iGluSnFR3 (maybe iGluSnFR2?), then iGluSnFR3 was developed and the best S72X mutations was transplated onto this scaffold and that is what is plotted in Fig1 S1?
%KD: If this^ is not right we need to explain it better
...
Surprisingly, the mRuby3 component responds to some amino acids at high millimolar concentrations, indicating a non-specific effect, potentially interacting with the C-terminal histidine tag (\FIGSUPP[Fig1]{f1S2}, panel B). Notably, this increase in fluorescence is still an order of magnitude lower than the green fluorescence response and at amino acid concentrations that are unlikely to be achieved in most cell types.
...
%LS- Reordered the next two sentences so the panels are in sequence
...
Identical to \FIG{Fig2}, panel E but with fewer glutamine concentrations and more well replicates.
AU, arbitrary unit.
...
Furthermore, we found that adding 1 mM asparagine delayed the decrease in sensor signal, suggesting that when asparagine can be salvaged from the media it diverts glutamine consumption that would otherwise be purposed for asparagine synthesis.
...
Using our sensor, we have shown that the time to reach steady-state can be much longer and dependents on the pharmacological treatment.
...
%KD: Assume this is the word document sequence. We should upload that as a supplementary file.
