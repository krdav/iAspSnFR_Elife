...
(A) Switching iGluSnFR3 specificity from glutamate to aspartate using S72X library (left) and S72P, S27X library (right).
Titrations with aspartate (solid lines) and glutamate (dashed lines) using bacterial lysate.
...
%KD: mission wavelength was varied from from 510 to 600??? Looks like from 535 to 600
...
RFP normalized iAspSnFR signal change over time following rotenone treatment of live cells.
...
(B) HT1080 cells using an RFP fused to iAspSnFR (iAspSnFR.mRuby3) for normalization.
...
RFP normalized iAspSnFR signal change over time following glutamine depletion in H1299 cells.
...
\label{fig:Fig3}
\figsupp[iAspSnFR signal does not correlate with glutamate concentration.]{
RFP normalized iAspSnFR signal, following various perturbations to live cells, is not correlated with the LCMS measured intracellular glutamate concentration.
...
%KD: Did they use iGluSnFR or iGluSnFR3?
...
On the other hand, our sensor appears to have higher glutamate and asparagine affinity; however, we found no correlation between intracellular glutamate concentration and sensor signal (\FIGSUPP[Fig3]{f3S1}).
